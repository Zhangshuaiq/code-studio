...
资源回收达到最大重试次数后进入 `deletion_failed`，管理员可在 `/admin/project-cleanups` 查看错误、填写确认备注并重新入队。确认会记录操作者和时间，并消除“未确认失败”健康告警；历史失败记录仍保留。如果集群已永久移除，系统设置管理员可输入完整项目名及处置依据，显式授权跳过远端部署停止并继续回收；授权人、时间和原因会随项目保留到最终删除前。该高风险入口仅用于确认运行目标已不存在的场景。

删除项目时若数据库仍记录部署为 `running`，平台会先幂等停止实际运行资源并复核数据库状态，而不是要求用户依赖可能延迟的状态页面手工停止。Kubernetes/Docker 资源已经不存在按停止成功处理；连接目标失败或删除资源失败时，删除请求仍进入 `deleting`，资源回收 Worker 会先按退避策略重试停止部署，确认后再删除文件和数据库记录，达到上限才进入 `deletion_failed`，避免瞬时网络故障卡住用户或遗留孤儿资源。仍在 `building` 的部署继续阻止首次删除，防止构建完成后重新写回运行状态。
