...
新建项目支持空白创建或从 HTTPS Git 仓库导入，也可在项目卡片或工作区“历史”面板中后续配置仓库。仓库地址属于项目，项目记录中不保存用户名或 PAT。
...
创建项目时可选择“创建新项目”或“从 Git 导入”。Git 模式会在返回成功前实际拉取 HTTPS 仓库，而不是只保存仓库地址；未填写默认分支时，系统根据远程 `HEAD` 自动识别并持久化实际分支。公开仓库支持匿名拉取，私有仓库使用当前用户按 Git 主机加密保存的 Token。无法识别远程默认分支时需要手动填写；导入失败会清理半成品项目，异常中断并停留超过 30 分钟的 `importing` 项目由资源回收 Worker 接管。
