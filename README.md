...
项目访问层统一执行生命周期门禁：`import_queued/importing` 不能创建或访问用户会话，`import_failed` 不能进入生成、文件、Git、预览和部署等工作区流程。失败项目仍允许具备项目管理权限的成员查看和修正仓库地址或分支，之后由项目所有者重试。具备 `system-setting:manage` 权限的管理员可在“管理后台 → 项目任务”分页查看排队、执行中及失败的导入任务，包括创建人、仓库、分支、尝试次数、租约和脱敏错误，并可将失败任务重新排队；所有重试均记录审计日志。
