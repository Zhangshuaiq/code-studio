...
export function PlatformHealthPage() {
  ...
  const enabledDependencies = data ? Object.values(data.dependencies).filter((x) => !x.disabled) : [];
  const online = enabledDependencies.filter((x) => x.available).length;
...
function DependencyCard({ label, description, icon, health }: { label: string; description: string; icon: React.ReactNode; health?: DependencyHealth }) { const disabled=health?.disabled; return <section className="card p-5"><div className="flex items-start justify-between"><div className="flex gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10">{icon}</span><div><h2 className="font-bold">{label}</h2><p className="mt-0.5 text-xs text-muted">{description}</p></div></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${disabled?'bg-slate-100 text-slate-500 dark:bg-slate-800':health?.available ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10' : 'bg-red-50 text-red-600 dark:bg-red-500/10'}`}>{health ? disabled?'未启用':health.available ? '正常' : '异常' : '检测中'}</span></div><div className="mt-5 grid grid-cols-2 gap-3"><Metric label="探测延迟" value={disabled?'—':health?.latencyMs === undefined ? '—' : `${health.latencyMs} ms`} /><Metric label="状态" value={disabled?'未配置':health?.available ? '可连接' : '不可连接'} /></div>{health?.error && <div className="mt-3 rounded-xl bg-red-50 p-3 text-xs text-red-600 dark:bg-red-950/30">{health.error}</div>}<Details health={health} /></section>; }
