...
import { ArrowLeft, BetweenHorizontalEnd, BetweenVerticalEnd, Bold, Code2, Columns3, Heading1, Heading2, Heading3, Italic, List, ListOrdered, Quote, Rows3, Save, Table2 } from "lucide-react";
...
export function RequirementDocumentPage() {
  ...
  useEffect(() => {
    ...
    const hover = (event: MouseEvent) => {
      const cell = (event.target as Element).closest("td,th") as HTMLElement | null;
      root.classList.toggle("row-resize-cursor", !!cell && Math.abs(event.clientY - cell.getBoundingClientRect().bottom) <= 6);
    };
    const down = (event: MouseEvent) => {
      const cell = (event.target as Element).closest("td,th") as HTMLElement | null;
      if (!cell || Math.abs(event.clientY - cell.getBoundingClientRect().bottom) > 6) return;
      event.preventDefault();
      const cells = Array.from(cell.parentElement?.children || []) as HTMLElement[];
      const startY = event.clientY; const startHeight = cell.getBoundingClientRect().height;
      const move = (next: MouseEvent) => cells.forEach(item => { item.style.height = `${Math.max(42, startHeight + next.clientY - startY)}px`; });
      const up = () => { document.removeEventListener("mousemove", move); document.removeEventListener("mouseup", up); root.classList.remove("row-resize-cursor"); };
      document.addEventListener("mousemove", move); document.addEventListener("mouseup", up);
    };
    root.addEventListener("mousemove", hover); root.addEventListener("mousedown", down);
    return () => { root.removeEventListener("mousemove", hover); root.removeEventListener("mousedown", down); };
  }, [editor, editable]);
  ...
  return <main className="flex h-screen min-h-0 flex-col overflow-hidden bg-white dark:bg-slate-950">
    ...
    <div className="flex shrink-0 flex-wrap items-center gap-1 border-b border-slate-200 bg-slate-50 px-4 py-2 dark:border-slate-800 dark:bg-slate-900/70">{tools.map(([Icon,label,action])=><button key={label} disabled={!editable} className="icon-btn" title={label} aria-label={label} onClick={action}><Icon size={15}/></button>)}{editor?.isActive("table")&&<><span className="mx-2 h-5 w-px bg-slate-300 dark:bg-slate-700"/><button className="icon-btn" title="在下方新增行" aria-label="在下方新增行" disabled={!editable} onClick={()=>editor.chain().focus().addRowAfter().run()}><BetweenHorizontalEnd size={16}/></button><button className="icon-btn" title="在右侧新增列" aria-label="在右侧新增列" disabled={!editable} onClick={()=>editor.chain().focus().addColumnAfter().run()}><BetweenVerticalEnd size={16}/></button><button className="icon-btn text-red-500" title="删除当前行" aria-label="删除当前行" disabled={!editable||!editor.can().deleteRow()} onClick={()=>editor.chain().focus().deleteRow().run()}><Rows3 size={16}/></button><button className="icon-btn text-red-500" title="删除当前列" aria-label="删除当前列" disabled={!editable||!editor.can().deleteColumn()} onClick={()=>editor.chain().focus().deleteColumn().run()}><Columns3 size={16}/></button></>}<span className="ml-2 text-[10px] text-muted">输入 #、##、-、1.、&gt; 后按空格可即时转换</span></div>
