...
import { ArrowLeft, Bold, Code2, Heading1, Heading2, Heading3, Italic, List, ListOrdered, Quote, Save, Table2 } from "lucide-react";
...
export function RequirementDocumentPage() {
  ...
  const [content, setContent] = useState(""); const [summary, setSummary] = useState("");
  ...
  const editor = useEditor({ extensions: [StarterKit, TableKit.configure({ table: { resizable: true } }), Placeholder.configure({ placeholder: "输入 # 后按空格创建一级标题…" }), Markdown], content: "", contentType: "markdown", editable, editorProps: { attributes: { class: "yuque-editor min-h-full px-10 py-8 outline-none" } }, onUpdate: ({ editor: e }) => setContent(e.getMarkdown()) });
  ...
  return <main className="flex h-screen min-h-0 flex-col overflow-hidden bg-white dark:bg-slate-950">
    ...
    <div className="flex shrink-0 items-center gap-1 border-b border-slate-200 bg-slate-50 px-4 py-2 dark:border-slate-800 dark:bg-slate-900/70">{tools.map(([Icon,label,action])=><button key={label} disabled={!editable} className="icon-btn" title={label} onClick={action}><Icon size={15}/></button>)}<span className="ml-2 text-[10px] text-muted">输入 #、##、-、1.、&gt; 后按空格可即时转换</span></div>
