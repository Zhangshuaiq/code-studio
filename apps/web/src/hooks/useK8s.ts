...
export function useDeleteClusterPod(targetId: string | undefined) {
  ...
  return useMutation({
    mutationFn: ({ namespace, name }: { namespace: string; name: string }) =>
      api.delete(
        `/k8s/${targetId}/namespaces/${encodeURIComponent(namespace)}/pods/${encodeURIComponent(name)}`,
      ),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["k8s-cluster-overview", targetId] }),
  });
}
...
export function useClusterDeploymentActions(targetId: string | undefined) {
  ...
  const scale = useMutation({
    mutationFn: ({
      ...
      api.post(
        `/k8s/${targetId}/namespaces/${encodeURIComponent(namespace)}/deployments/${encodeURIComponent(name)}/scale`,
        { replicas },
      ),
    onSuccess: refresh,
  });
  const restart = useMutation({
    mutationFn: ({ namespace, name }: { namespace: string; name: string }) =>
      api.post(
        `/k8s/${targetId}/namespaces/${encodeURIComponent(namespace)}/deployments/${encodeURIComponent(name)}/restart`,
      ),
    onSuccess: refresh,
  });
  const remove = useMutation({
    mutationFn: ({ namespace, name }: { namespace: string; name: string }) =>
      api.delete(
        `/k8s/${targetId}/namespaces/${encodeURIComponent(namespace)}/deployments/${encodeURIComponent(name)}`,
      ),
    onSuccess: refresh,
  });
...
export function useClusterNetworkActions(targetId: string | undefined) {
  const qc = useQueryClient();
  const refresh = () =>
    qc.invalidateQueries({ queryKey: ["k8s-cluster-overview", targetId] });
  const removeService = useMutation({
    mutationFn: ({ namespace, name }: { namespace: string; name: string }) =>
      api.delete(
        `/k8s/${targetId}/namespaces/${encodeURIComponent(namespace)}/services/${encodeURIComponent(name)}`,
      ),
    onSuccess: refresh,
  });
  const removeIngress = useMutation({
    mutationFn: ({ namespace, name }: { namespace: string; name: string }) =>
      api.delete(
        `/k8s/${targetId}/namespaces/${encodeURIComponent(namespace)}/ingresses/${encodeURIComponent(name)}`,
      ),
    onSuccess: refresh,
  });
  return { removeService, removeIngress };
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get(
        `/k8s/${targetId}/namespaces/${namespace}/pods`,
      );
      return res.data as Array<{
        name: string;
        namespace: string;
...
export function usePod(
  ...
  return useQuery<PodDetail | null>({
    ...
    queryFn: async () => {
      ...
      const res = await api.get(
        `/k8s/${targetId}/namespaces/${namespace}/pods/${name}`,
      );
      return res.data;
    },
...
export function usePodLogs(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get(
        `/k8s/${targetId}/namespaces/${namespace}/pods/${name}/logs`,
        { params: { container, previous, tail: 500 } },
      );
...
export function useDeployments(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!targetId || !namespace) return [];
      const res = await api.get(
        `/k8s/${targetId}/namespaces/${namespace}/deployments`,
      );
      return res.data as Array<{
        name: string;
        namespace: string;
...
export function useServices(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!targetId || !namespace) return [];
      const res = await api.get(
        `/k8s/${targetId}/namespaces/${namespace}/services`,
      );
      return res.data as Array<{
        name: string;
        namespace: string;
