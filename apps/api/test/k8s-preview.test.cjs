...
test('Kubernetes preview binding accepts BuildKit repository and HTTPS context', () => {
  const config = validatePreviewBindingConfig({
    ...
    sourceBaseUrl: 'https://git.example.com/team/app.git',
...
test('Kubernetes preview binding rejects missing image and unsafe env', () => {
  ...
  assert.throws(() => validatePreviewBindingConfig({ imageRepository: 'repo/app', sourceBaseUrl: 'file:///tmp/app', baseDomain: 'preview.example.com' }), /sourceBaseUrl/);
