...
export class ScheduledTaskService implements OnModuleInit, OnModuleDestroy {
  ...
  async createTask(input: CreateScheduledTaskDto, user: AuthUser) {
    ...
    const scheduleType = input.scheduleType && ['immediate', 'once', 'cron'].includes(input.scheduleType) ? input.scheduleType : 'immediate';
  ...
  private async createExecution(task: RunnableTask, triggerType: 'manual' | 'schedule') {
    ...
    return this.prisma.javaTaskExecution.create({ data: { taskId: task.id, triggerType, parametersSnapshot: task.parameters === null ? Prisma.JsonNull : task.parameters as Prisma.InputJsonValue, handlerVersion: task.handler.version, scheduledAt: new Date(), traceId: randomUUID() } });
  }
