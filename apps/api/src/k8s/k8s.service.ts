...
export function previewNamespaceName(prefix: string, teamId: string) {
  ...
  const name = `${normalizedPrefix}-${normalizedTeam}`
    .slice(0, 63)
    .replace(/-+$/g, "");
  if (!normalizedTeam || !/^[a-z0-9]([-a-z0-9]*[a-z0-9])?$/.test(name)) {
    throw new BadRequestException({
      code: "K8S_PREVIEW_NAMESPACE_INVALID",
      message: "无法生成合法的预览 Namespace 名称",
    });
  }
  return name;
}
...
export class K8sService {
  ...
  async getAuthorizedClients(targetId: string, userId: string) {
    ...
    if (!target) {
      throw new NotFoundException({
        code: "DEPLOY_TARGET_NOT_FOUND_OR_INACCESSIBLE",
        message: "部署目标不存在或无权访问",
      });
    }

    if (target.kind !== "k8s") {
      throw new BadRequestException({
        code: "DEPLOY_TARGET_KIND_MISMATCH",
        message: "所选部署目标不是 Kubernetes 集群",
      });
    }

    if (!target.enabled) {
      throw new BadRequestException({
        code: "DEPLOY_TARGET_DISABLED",
        message: "所选部署目标已停用",
      });
    }
  ...
  async provisionPreviewNamespace(
    targetId: string,
    userId: string,
    teamId: string,
  ) {
    ...
    if (!team)
      throw new ForbiddenException({
        code: "K8S_TEAM_ACCESS_DENIED",
        message: "不属于该项目组",
      });
    const { coreApi, networkingApi, target } = await this.getAuthorizedClients(
      targetId,
      userId,
    );
    if (!target.purposes.includes("preview")) {
      throw new BadRequestException({
        code: "K8S_PREVIEW_PURPOSE_DISABLED",
        message: "该 Kubernetes 目标未启用预览用途",
      });
    }
    if (target.scope === "team" && target.teamId !== teamId) {
      throw new ForbiddenException({
        code: "K8S_TARGET_TEAM_MISMATCH",
        message: "运行资源与项目组不匹配",
      });
    }

    const namespace = previewNamespaceName(
      this.config.get("PREVIEW_NAMESPACE_PREFIX", "codegen-preview"),
      teamId,
    );
    const labels = {
        ...
        coreApi.replaceNamespace({
          ...
          body: {
            ...
            metadata: {
              ...current.metadata,
              name: namespace,
              labels: { ...current.metadata?.labels, ...labels },
            },
          },
        }),
    );
    ...
    await upsertResource(
      () =>
        coreApi.readNamespacedServiceAccount({
          name: serviceAccount,
          namespace,
        }),
      () =>
    ...
    const hard = {
      ...
      "requests.memory": this.config.get(
        "PREVIEW_NAMESPACE_REQUESTS_MEMORY",
        "32Gi",
      ),
      "limits.cpu": this.config.get("PREVIEW_NAMESPACE_LIMITS_CPU", "32"),
      "limits.memory": this.config.get(
        "PREVIEW_NAMESPACE_LIMITS_MEMORY",
        "64Gi",
      ),
      persistentvolumeclaims: this.config.get(
        "PREVIEW_NAMESPACE_MAX_PVCS",
        "10",
      ),
    ...
    await upsertResource(
      ...
      () =>
        coreApi.createNamespacedResourceQuota({
          namespace,
          body: {
            metadata: { name: quotaName, namespace, labels },
            spec: { hard },
          },
        }),
      (current) =>
    ...
    await upsertResource(
      ...
      () =>
        coreApi.createNamespacedLimitRange({
          namespace,
          body: {
            metadata: { name: limitName, namespace, labels },
            spec: { limits },
          },
        }),
      (current) =>
        coreApi.replaceNamespacedLimitRange({
          name: limitName,
          namespace,
          body: {
            metadata: {
  ...
  async clusterOverview(targetId: string, userId: string) {
    try {
      const { coreApi, appsApi, networkingApi, target } =
        await this.getAuthorizedClients(targetId, userId);
      const [pods, namespaces, deployments, services, ingresses] =
        await Promise.all([
      ...
      return {
        ...
        deployments: deployments.items.map((deployment) => ({
          ...
          conditions: (deployment.status?.conditions || []).map(
            (condition) => ({
              type: condition.type,
        ...
        services: services.items.map((service) => ({
          ...
          ports: (service.spec?.ports || []).map((port) => ({
            ...
            port: port.port,
            targetPort:
              port.targetPort == null ? null : String(port.targetPort),
            protocol: port.protocol || "TCP",
            nodePort: port.nodePort || null,
          })),
          selector: service.spec?.selector || {},
          createdAt: service.metadata?.creationTimestamp || null,
        })),
        ingresses: ingresses.items.map((ingress) => ({
          ...
          hosts: (ingress.spec?.rules || [])
            .map((rule) => rule.host)
            .filter(Boolean),
          tlsHosts: (ingress.spec?.tls || []).flatMap((tls) => tls.hosts || []),
          addresses: (ingress.status?.loadBalancer?.ingress || [])
            .map((address) => address.ip || address.hostname)
            .filter(Boolean),
          createdAt: ingress.metadata?.creationTimestamp || null,
        })),
  ...
  async deleteIngress(
    ...
    const { networkingApi } = await this.getAuthorizedClients(targetId, userId);
  ...
  private containerState(state?: k8s.V1ContainerState) {
    if (state?.running)
      return { type: "running", startedAt: state.running.startedAt || null };
    if (state?.waiting)
      return {
        type: "waiting",
    ...
    if (state?.terminated)
      return {
        ...
        startedAt: state.terminated.startedAt || null,
